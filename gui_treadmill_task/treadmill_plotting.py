...
class TreadmillTask_plot(QtGui.QWidget): #similar to class Task_plot from gui/plotting.py
    ...
    def set_state_machine(self,sm_info):
        ...
        #for test.py only: TO BE REPLACED BY THE LINES COMMENTED ABOVE FOR THE TREADMILL TASK
        #use to reset the plot and the run clock at the beginning of the intertrial state and with every motion event
        self.event_IDs=list(sm_info['events'].values())
        self.state_IDs = list(sm_info['states'].values())
        self.motion_event_ID=sm_info['events']['motion'] 
        self.intertrial_ID=sm_info['states']['intertrial']
    ...
    def run_start(self, recording):
        ...
        self.n_intertrial=0
        self.trial=False
        self.intertrial=False #change to true in the treadmill task gui file
    ...
    def process_data(self,new_data):
        ...
        #for test.py only- REPLACED BY THE LINES COMMENTED ABOVE FOR THE TREADMILL TASK
        #used to reset the plot and run clock every trial (motion event here) and intertrial:
        self.new= [nd for nd in new_data if nd[0] == 'D' and nd[2] in self.event_IDs]+[nd for nd in new_data if nd[0] == 'D' and nd[2] in self.state_IDs]
        if self.new:
            for ne in self.new:
                if ne[2]==self.motion_event_ID:
                    self.position_plot.clear_plot()
                    self.motion_event_time=time.time()
                    self.n_trial+=1 
                    self.trial=True
                    self.intertrial=False
                elif ne[2]==self.intertrial_ID:
                    self.position_plot.clear_plot()
                    self.intertrial_start_time=time.time()
                    self.n_intertrial+=1 
                    self.trial=False
        ...
        if not self.pause_button.isChecked():
            ...
            if self.trial: 
                if self.n_trial==0:
                    ...
                    motion_time=time.time()-self.motion_event_time  #to be replaced by trial_time=time.time()-self.trial_start_time
                    self.position_plot.update()
                    self.run_clock.update_trial_clock(run_time, motion_time, self.n_trial)  #to replace motion_time by trial_time
...
class Treadmill_run_clock(): 
    ...
    def update_trial_clock(self, run_time, motion_time, n_trial):  #replace motion_time by trial_time
        self.clock_text.setText('Session time: {}\nTrial number: {}\nTrial time: {}'.format(str(timedelta(seconds=run_time))[:7],n_trial,str(timedelta(seconds=motion_time))[:7]))  #replace motion_time by trial_time
